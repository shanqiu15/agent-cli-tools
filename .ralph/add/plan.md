...
- `packages/ocr_tool/src/ocr_tool/engines/google.py` — basic ADC-only implementation using `document_text_detection`. Creates `ImageAnnotatorClient()` without client options (line 50). Needs API key auth added.
- `packages/ocr_tool/pyproject.toml` — already includes `google-cloud-vision>=3.7` dependency.
- `packages/ocr_tool/tests/test_engines/test_google.py` — 4 tests covering ADC-only credential check, missing file, successful extraction, and empty result. Needs API key auth tests added.
...
1. **Update the existing engine** at `packages/ocr_tool/src/ocr_tool/engines/google.py` to add API key authentication. Refactor the credential check (currently lines 25-38) to look for API keys first, then fall back to ADC. Refactor client creation (currently line 50) to pass `client_options={"api_key": key}` when an API key is found.
...
4. **Authentication priority**: `GOOGLE_API_KEY` > `GOOGLE_PLACES_API_KEY` > `GEMINI_API_KEY` > ADC (`GOOGLE_APPLICATION_CREDENTIALS` or `google.auth.default()`). Raise `OcrError` with code `MISSING_CREDENTIALS` only if all methods fail.
