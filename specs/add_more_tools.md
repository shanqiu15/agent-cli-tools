...
3. **code_execution_tool** -- sandbox layer, platform-specific but no external API (see `specs/sandbox.md`)
4. **vision_tool** -- first tool with external API providers
5. **transcription_tool** -- similar provider pattern to vision
6. **tts_tool** -- similar provider pattern, adds edge-tts dependency
7. **skill_tool** -- most complex (CRUD + validation + filesystem structure)
